...
const HotelAmenities = () => {
  ...
  useEffect(() => {
    setAmenitiesProvided(["WiFi", "Air conditioning", "Cable TV", "Essentials", "Parking"]);
    setAllAmenities([{
      name: "WiFi",
  ...
  return <View style={styles.container}>
      <FlatList data={allAmenities} renderItem={({
      item
    }) => <View style={styles.amenityContainer}>
            <Image source={getIcon(item.name)} style={styles.icon} />
            <View>
              <Text style={isProvided(item.name) ? styles.amenityName : styles.crossed}>
                {item.name}
              </Text>
              {item.description && isProvided(item.name) ? <Text style={styles.description}>{item.description}</Text> : null}
              {!isProvided(item.name) && <Text style={styles.description}>
                  The host hasn&apos;t reported a carbon monoxide detector on
                  the property
                </Text>}
            </View>
          </View>} keyExtractor={(item, index) => index.toString()} ListFooterComponent={() => <Button buttonText="Back" />} />
    </View>;
};
